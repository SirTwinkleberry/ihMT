...
from brainhack.meta import _Event, check_value_is_valid, _deg2rad, ScalarOrVector
...
class _PulseVector(ABC, _Event):
    ...
    def copy(self) -> _PulseVector:
        return self(**{key.replace('_vector_', ''): deepcopy(value) for key, value in self.__dict__.items() if '_vector_' in key})
    ...
    def gyromagneticFactor(self) -> ndarray[number]:
        if not hasattr(self, '_gyromagneticFactor'):
            self._gyromagneticFactor = atleast_1d(267513000.)
    ...
    def amplitudeIntegral(self) -> ndarray[number]:
        if not hasattr(self, '_amplitudeIntegral'):
            self._amplitudeIntegral = quad(self.value, 0, self.duration)[0] / self.duration
    ...
    def powerIntegral(self) -> ndarray[number]:
        if not hasattr(self, '_powerIntegral'):
            self._powerIntegral = quad(lambda t: self.value(t)**2, 0, self.duration)[0] / self.duration
    ...
    def b1peak(self) -> ndarray[number]:
        if not hasattr(self, '_b1peak'):
            self._b1peak = _deg2rad * self.flipAngle / (self.gyromagneticFactor * self.amplitudeIntegral * self.duration)
...
class TukeyVector(_PulseVector):
    ...
    def __init__(self, duration: ScalarOrVector, shape: ScalarOrVector, flipAngle: ScalarOrVector, offset: ScalarOrVector, *args: Any, **kwargs: Any):
        """_Tukey pulse class_
        ...
        duration, shape, flipAngle, offset = atleast_1d(duration), atleast_1d(shape), atleast_1d(flipAngle), atleast_1d(offset)
    ...
    def copy(self) -> TukeyVector:
        return TukeyVector(deepcopy(self._vector_duration), deepcopy(self._vector_shape), deepcopy(self._vector_flipAngle), deepcopy(self._vector_offset))
    ...
    def value(self, t: float) -> float:
        ...
        mask = (0 < dimless_time) & (dimless_time < .5 * self.shape)
        pulse[mask] =  0.5 * ( 1 + cos(pi * (-1 + 2 * dimless_time[mask] / self.shape[mask])) )

        mask = (.5 * self.shape <= dimless_time) & (dimless_time <= 1 - .5 * self.shape)
        pulse[mask] = 1

        mask = (1 > dimless_time) & (dimless_time > (1 - .5 * self.shape))
        pulse[mask] = 0.5 * ( 1 + cos(pi * (1 - 2 / self.shape[mask] + 2 * dimless_time[mask] / self.shape[mask])) )

        return pulse

    def _reshape(self):
        DD, SS, FF, OO = meshgrid(self._vector_duration, self._vector_shape, self._vector_flipAngle, self._vector_offset, indexing='ij', sparse=False)
    ...
    def amplitudeIntegral(self) -> ndarray[number]:
        if not hasattr(self, '_amplitudeIntegral'):
            self._amplitudeIntegral = 1 - .5 * self.shape
            self._changed('amplitudeIntegral')
        return self._amplitudeIntegral
