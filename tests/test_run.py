...
from ihmt.run import SingleRun
...
class TestRun_withoutExport(TestCase):
    ...
    def test_run_SingleRun_CM_noExport(self):
        ...
        try:
            output = check_output(
                [
                    "python",
                    str(Path(__file__).parents[1] / "ihmt" / "run.py"),
            ...
            self.assertEqual(
                ...
                str.encode(
                    "\n".join(
                        [
                            f"{key}: {str(sublist)}"
                            for key, sublist in CONFIG_SIMULATOR["compute"][
                                "ihMTR_CM"
                            ].items()
                        ]
                    )
    ...
    def test_run_SingleRun_CM_checkExport(self):
        ...
        try:
            ...
            mat = loadmat(
                Path(DEFAULT["run"]["outputDir"]).resolve()
                / (DEFAULT["run"]["filePrefix"] + "simulation.mat")
            )
            base = loadmat(
                Path(__file__).parent / "simulator_test_singleRun_CM_checkExport.mat"
            )
    ...
    def test_run_SingleRun_ALT_checkExport(self):
        ...
        try:
            ...
            base = loadmat(
                Path(__file__).parent / "simulator_test_singleRun_ALT_checkExport.mat"
            )
    ...
    def test_run_SingleRun_BP_checkExport(self):
        ...
        try:
            ...
            base = loadmat(
                Path(__file__).parent / "simulator_test_singleRun_BP_checkExport.mat"
            )
    ...
    def test_run_SingleRun_wrongSignal(self):
        ...
        with self.assertRaises(ValueError):
            try:
                ...
                if "Incorrect signal flag. Must be any one or combinations of" in str(
                    e.output
                ):
                    raise ValueError(
                        "Incorrect signal flag. Must be any one or combinations of ..."
                    )
                raise RuntimeError(e.output.decode("utf-8"))
    ...
    def test_run_wrongNumberOfArguments(self):
        with self.assertRaises(SyntaxError):
            try:
                ...
                if "Running command with the wrong number of arguments." in str(
                    e.output
                ):
                    raise SyntaxError(
...
class TestSingleRun(TestCase):
    ...
    def test_singleRun_CM_checkExport(self):
        ...
        mat = loadmat(
            Path(params["outputDir"]).resolve()
            / (params["filePrefix"] + "simulation.mat")
        )
        base = loadmat(
            Path(__file__).parent / "simulator_test_singleRun_CM_checkExport.mat"
        )
