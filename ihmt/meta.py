        ...
        with errstate(invalid="ignore", divide="ignore"):
            if isinstance(other, CompositeDictionary):
                return CompositeDictionary(
                    {
                        key: operator(self[key], other[key])
                        for key in self.keys() | other.keys()
                    }
                )
            else:
                return CompositeDictionary(
                    {key: operator(self[key], other) for key in self.keys()}
                )
...
class _Event:
    ...
    def onChange(self, attribute: str, callbacks: list[Callable]):
        ...
        if attribute not in onChanges.keys():
            logger.debug(
                f"Initializing `{attribute}` callback list to `_onChanges` for `{self.__class__.__name__}`."
            )
            onChanges[attribute] = []
        onChanges[attribute].extend(callbacks)
        logger.debug(
            f"Extended `{attribute}` callback list with `{callbacks}` for `{self.__class__.__name__}`."
        )
    ...
    def _changed(self, attribute: str):
        ...
        if attribute in onChanges.keys():
            logger.debug(
                f"Parsing callbacks for `{attribute}` in `{self.__class__.__name__}`."
            )
    ...
    def _reset_computed_attributes(self, attributelist: list[str]):
        for attribute in attributelist:
            if hasattr(
                self, f"_{attribute}"
            ):  # prefix `_` to avoid calling getter method
                try:
                    delattr(
                        self, f"{attribute}"
                    )  # No prefix `_` to ensure calling deleter method
                    logger.debug(
                        f"Called for deletion of `{attribute}`. `_{attribute}` was deleted in `{self.__class__.__name__}`."
                    )
                except AttributeError as e:
                    logger.debug(
                        f"Called for deletion of `{attribute}` but exception ensued:"
                    )
                    logger.debug(e)
            else:
                logger.debug(
                    f"Called for deletion of `{attribute}` but `_{attribute}` was not found in `{self.__class__.__name__}`."
                )
...
class Angle(_BaseUnit):
    ...
    def sec(self) -> float:
        if not hasattr(self, "__sec"):
            self.__sec = 1.0 / self.cos
        return self.__sec

    @property
    def csc(self) -> float:
        if not hasattr(self, "__csc"):
            self.__csc = 1.0 / self.sin
        return self.__csc

    @property
    def cot(self) -> float:
        if not hasattr(self, "__cot"):
            self.__cot = 1.0 / self.tan
        return self.__cot
