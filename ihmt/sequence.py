...
from ihmt.meta import _Event, Signal, check_value_is_valid
...
class Sequence(_Event):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self.onChange(
            ...
            [
                lambda: self._reset_computed_attributes(
                    ["duration_preparation", "duration_recovery"]
                ),
        ...
        self.onChange(
            "tr",
            [
                lambda: self._reset_computed_attributes(["duration_recovery"]),
                self._check_against_tr,
            ],
        )
    ...
    def _check_against_tr(self):
        if (
            ...
            and hasattr(self, "_dt_lastBurst")  # Prep parameters
    ...
    def _check_against_tr_burst(self):
        if (
            hasattr(self, "_TR_burst")
            and hasattr(self, "_N_pulse")
            and hasattr(self, "_dt_interPulse")
        ):
    ...
    def _check_against_N_pulse_multiplicity(self):
        if (
            hasattr(self, "_N_pulse")
            and hasattr(self, "_N_pulsePerOffset")
            and hasattr(self, "_signal")
        ):
            if (Signal.MTd_ALT in self.signal) and (
                ((0.5 * self.N_pulse) % self.N_pulsePerOffset) != 0
            ):
    ...
    def duration_preparation(self):
        if not hasattr(self, "_duration_preparation"):
            self._duration_preparation = (
                self.N_burst - 1
            ) * self.TR_burst + self.dt_lastBurst
    ...
    def duration_recovery(self):
        if not hasattr(self, "_duration_recovery"):
            self._duration_recovery = (
                self.tr - self.duration_readout - self.duration_preparation
            )
