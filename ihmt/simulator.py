...
class Simulator(_Event):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self.onChange(
            "pulse",
            [
                lambda: setattr(self.sequence, "pulse", self.pulse),
                lambda: setattr(self.system, "pulse", self.pulse),
    ...
    def copy(self) -> Simulator:
        ...
        return Simulator(
            system, sequence, self.output_vectorSlice, self.export_readMatrix
        )
    ...
    def SteadyState(self) -> dict[str, ndarray[number]]:
        ...
        mat_REX = vstack(
            [
                hstack([sys.relaxation, sys.magnetization_recovery]),
        ...
        if (
            ...
            if (Signal.MTs in seq.signal) or (Signal.MTd_ALT in seq.signal):
                evol_rf_singleSat_Positive: ndarray[number] = expm(
                    vstack(
                        [
                            hstack(
                                [
                                    sys.poolBound_Rrf_singleSat_Positive
                                    + sys.poolFree_Rrf
                                    + sys.relaxation,
                                    sys.magnetization_recovery,
                                ]
                            ),
                            zeros((1, 1 + sys.N_poolFree + 2 * sys.N_poolBound)),
                        ]
                ...
                evol_rf_singleSat_Negative: ndarray[number] = expm(
                    vstack(
                        [
                            hstack(
                                [
                                    sys.poolBound_Rrf_singleSat_Negative
                                    + sys.poolFree_Rrf
                                    + sys.relaxation,
                                    sys.magnetization_recovery,
                                ]
                ...
                if Signal.MTs_Positive in seq.signal:
                    ...
                    v_MTs_Positive = eig(
                        round(evol_dummyRAGE @ (evol_MTsat_single @ evol_RAGE), 16)
                    )[1][:, -1]
                ...
                if Signal.MTd_ALT in seq.signal:
                    evol_MTsat_dual_ALT: ndarray[number] = (
                        ...
                        @ matrix_power(
                            matrix_power(
                                evol_relax_interPulse @ evol_rf_singleSat_Negative,
                                seq.N_pulsePerOffset,
                            )
                            @ matrix_power(
                                evol_relax_interPulse @ evol_rf_singleSat_Positive,
                                seq.N_pulsePerOffset,
                            ),
                            int(0.5 * seq.N_pulse / seq.N_pulsePerOffset),
                        )
                        @ matrix_power(
                            evol_relax_TR_burst
                            @ matrix_power(
                    ...
                    v_MTd_ALT = eig(
                        round(evol_dummyRAGE @ (evol_MTsat_dual_ALT @ evol_RAGE), 16)
                    )[1][:, -1]
