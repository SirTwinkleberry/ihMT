...
from ihmt.meta import _Event, ScalarOrVector, ScalarOrMatrix, check_value_is_valid
from ihmt.pulse import Pulse
...
class System(_Event):
    ...
    _poolFree_Rrf: ndarray
    _poolFree_M0: ndarray
    _poolFree_T1: ndarray
    _poolFree_T2: ndarray

    _poolFreeBound_exchangeRate: ndarray

    _poolBound_Rrf_singleSat_Positive: ndarray
    _poolBound_Rrf_singleSat_Negative: ndarray
    _poolBound_Rrf_dualSat: ndarray
    _poolBound_lineshapeAsymmetry: ndarray
    _poolBound_M0: ndarray
    _poolBound_T1: ndarray
    _poolBound_T2: ndarray
    _poolBound_T1D: ndarray
    _poolBound_omegaLocalField: ndarray

    _magnetization_recovery: ndarray
    _relaxation: ndarray
    ...
    def __init__(
        ...
        poolBound_lineshapeAsymmetry: ScalarOrVector,
        *args: Any,
        **kwargs: Any,
    ):
        """_summary_
        ...
        self.onChange(
            ...
            [
                lambda: self._reset_computed_attributes(
                    ["N_pools", "magnetization_recovery", "relaxation"]
                )
            ],
        )
        self.onChange(
            "poolFree_M0",
            [
                lambda: self._check_pool_dimension_compatibility(
                    "poolFree_M0", self.poolFree_M0.shape, "free"
                ),
                lambda: self._reset_computed_attributes(
                    ["N_poolFree", "magnetization_recovery", "relaxation"]
                ),
            ],
        )
        self.onChange(
            ...
            [
                lambda: self._check_pool_dimension_compatibility(
                    "poolFree_T1", self.poolFree_T1.shape, "free"
                ),
                lambda: self._reset_computed_attributes(
                    ["N_poolFree", "magnetization_recovery", "relaxation"]
                ),
        ...
        self.onChange(
            ...
            [
                lambda: self._check_pool_dimension_compatibility(
                    "poolFree_T2", self.poolFree_T2.shape, "free"
                ),
                lambda: self._reset_computed_attributes(["N_poolFree", "poolFree_Rrf"]),
            ],
        )
        ...
        self.onChange(
            ...
            [
                lambda: self._check_pool_dimension_compatibility(
                    "poolFreeBound_exchangeRate",
                    self.poolFreeBound_exchangeRate.shape,
                    "both",
                ),
                lambda: self._reset_computed_attributes(
                    ["N_poolFree", "N_poolBound", "relaxation"]
                ),
            ],
        )
        ...
        self.onChange(
            ...
            [
                lambda: self._check_pool_dimension_compatibility(
                    "poolBound_M0", self.poolBound_M0.shape, "bound"
                ),
                lambda: self._reset_computed_attributes(
                    ["N_poolBound", "magnetization_recovery", "relaxation"]
                ),
            ],
        )
        self.onChange(
            ...
            [
                lambda: self._check_pool_dimension_compatibility(
                    "poolBound_T1", self.poolBound_T1.shape, "bound"
                ),
                lambda: self._reset_computed_attributes(
                    ["N_poolBound", "magnetization_recovery", "relaxation"]
                ),
        ...
        self.onChange(
            ...
            [
                lambda: self._check_pool_dimension_compatibility(
                    "poolBound_T2", self.poolBound_T2.shape, "bound"
                ),
                lambda: self._reset_computed_attributes(
                    [
                        "N_poolBound",
        ...
        self.onChange(
            ...
            [
                lambda: self._check_pool_dimension_compatibility(
                    "poolBound_T1D", self.poolBound_T1D.shape, "bound"
                ),
                lambda: self._reset_computed_attributes(["N_poolBound", "relaxation"]),
            ],
        )
        self.onChange(
            "poolBound_lineshapeAsymmetry",
            [
                lambda: self._check_pool_dimension_compatibility(
                    "poolBound_lineshapeAsymmetry",
                    self.poolBound_lineshapeAsymmetry.shape,
                    "bound",
                ),
                lambda: self._reset_computed_attributes(
                    [
        ...
        self.onChange(
            ...
            [
                lambda: self._check_pool_dimension_compatibility(
                    "poolBound_omegaLocalField",
                    self.poolBound_omegaLocalField.shape,
                    "bound",
                ),
                lambda: self._reset_computed_attributes(
                    [
        ...
        self._check_pool_dimension_compatibility(
            "poolFree_M0", self.poolFree_M0.shape, "free"
        )
        self._check_pool_dimension_compatibility(
            "poolFree_T1", self.poolFree_T1.shape, "free"
        ...
        self._check_pool_dimension_compatibility(
            "poolFreeBound_exchangeRate", self.poolFreeBound_exchangeRate.shape, "both"
        )
        self._check_pool_dimension_compatibility(
            "poolBound_M0", self.poolBound_M0.shape, "bound"
        )
        ...
        self._check_pool_dimension_compatibility(
            "poolBound_omegaLocalField", self.poolBound_omegaLocalField.shape, "bound"
        )

    def copy(self) -> "System":
        return System(
            self.pulse.copy(),
            deepcopy(self.poolFree_M0),
            deepcopy(self.poolFree_T1),
            deepcopy(self.poolFree_T2),
            deepcopy(self.poolFreeBound_exchangeRate),
            deepcopy(self.poolBound_M0),
            deepcopy(self.poolBound_T1),
            deepcopy(self.poolBound_T2),
            deepcopy(self.poolBound_T1D),
            deepcopy(self.poolBound_lineshapeAsymmetry),
        )

    def _check_pool_dimension_compatibility(
        self, caller: str, shape: tuple[int, ...], pool: str
    ) -> bool:
        ...
        match pool:
            case "both":
                if len(shape) != 2:
                    raise ValueError(
                        f"Expected a shape of length 2, received `{shape}` of length {len(shape)}. Caller: {caller}."
                    )
                self._check_pool_dimension_compatibility(
                    "poolFreeBound_exchangeRate", (shape[0],), "free"
                )
                self._check_pool_dimension_compatibility(
                    "poolFreeBound_exchangeRate", (shape[1],), "bound"
                )
                return True

            case "free":
                attrlist = poolFree_attrs
                if hasattr(self, "_poolFreeBound_exchangeRate") and (
                    shape != self.poolFreeBound_exchangeRate.shape[slice(None, 1)]
                ):
                    raise ValueError(
                        f"Dimension mismatched between new free pool shape `{shape}` and exchange rate matrix column length `{self.poolFreeBound_exchangeRate.shape[slice(None, 1)]}`. Caller: {caller}."
                    )

            case "bound":
                attrlist = poolBound_attrs
                if hasattr(self, "_poolFreeBound_exchangeRate") and (
                    shape != self.poolFreeBound_exchangeRate.shape[slice(1, None)]
                ):
                    raise ValueError(
                        f"Dimension mismatched between new bound pool shape `{shape}` and exchange rate matrix row length `{self.poolFreeBound_exchangeRate.shape[slice(1, None)]}`. Caller: {caller}."
                    )

            case _:
                raise ValueError(
                    f"Pool attribute (`{pool}`) does not match available values: [`free`, `bound`, `both`]. Caller: {caller}."
                )
        ...
        for attr in attrlist:
            if hasattr(self, f"_{attr}"):
                if shape != getattr(self, attr).shape:
                    raise ValueError(
                        f"Dimension mismatched between new {pool} pool attribute shape `{shape}` and `{attr}` shape `{getattr(self, attr).shape}`. Caller: {caller}."
                    )
    ...
    def _compute_poolBound_RFabsorptionMatrices(self):
        """_summary_"""
        inv_omegaLocalField = 1.0 / self.poolBound_omegaLocalField
        ...
        superLorentzian: Callable[[ndarray, float], ndarray] = (
            self.PampelSuperLorentzian
        )
        # superLorentzian: Callable[[ndarray, float], ndarray] = self.SuperLorentzian if abs(self.pulse.offset) > 1000 else self.PampelSuperLorentzian
        ...
        if (self.poolBound_lineshapeAsymmetry != 0).any():
            poolBound_Rrf_Positive: ndarray = (
                ...
                * superLorentzian(
                    self.poolBound_T2,
                    self.pulse.offset - self.poolBound_lineshapeAsymmetry,
                )
            )
            poolBound_Rrf_Negative: ndarray = (
                0.5
                * norm_A
                * superLorentzian(
                    self.poolBound_T2,
        ...
        tmp_diag: list[ndarray] = [diag([1, elem]) for elem in diag_elements]
        tmp_anti: list[ndarray] = [
            fliplr(diag([angularFrequencyOffset, elem])) for elem in anti_elements
        ]

        diag_Positive: list[ndarray] = [
            -Rrf_scalar * mat
            for Rrf_scalar, mat in zip(poolBound_Rrf_Positive, tmp_diag)
        ]
        anti_Positive: list[ndarray] = [
            Rrf_scalar * mat
            for Rrf_scalar, mat in zip(poolBound_Rrf_Positive, tmp_anti)
        ]

        diag_Negative: list[ndarray] = [
            -Rrf_scalar * mat
            for Rrf_scalar, mat in zip(poolBound_Rrf_Negative, tmp_diag)
        ]
        anti_Negative: list[ndarray] = [
            -Rrf_scalar * mat
            for Rrf_scalar, mat in zip(poolBound_Rrf_Negative, tmp_anti)
        ]
        ...
        self._poolBound_Rrf_dualSat = block_diag(  # type: ignore
            zeros((self.N_poolFree, self.N_poolFree)),
            *[
                0.5 * (tmp_diag_Positive + tmp_diag_Negative)
                for tmp_diag_Positive, tmp_diag_Negative in zip(
                    diag_Positive, diag_Negative
                )
            ],
        )
        self._poolBound_Rrf_singleSat_Positive = block_diag(  # type: ignore
            zeros((self.N_poolFree, self.N_poolFree)),
            *[
                tmp_diag_Positive + tmp_anti_Positive
                for tmp_diag_Positive, tmp_anti_Positive in zip(
                    diag_Positive, anti_Positive
                )
            ],
        )
        self._poolBound_Rrf_singleSat_Negative = block_diag(  # type: ignore
            zeros((self.N_poolFree, self.N_poolFree)),
            *[
                tmp_diag_Negative + tmp_anti_Negative
                for tmp_diag_Negative, tmp_anti_Negative in zip(
                    diag_Negative, anti_Negative
                )
            ],
        )
    ...
    def Lorentzian(
        self, T2: ndarray, offsets: float | ndarray, *args: Any, **kwargs: Any
    ) -> ndarray:
        """_summary_
    ...
    def SuperLorentzian(
        ...
        reduced_R2_square = 0.25 / (T2 * T2)
        return sqrt(2 * pi) * array(
            [
                quad(
                    lambda cos_theta: self._Spherical(cos_theta, offset, r2sq, 0), 0, 1
                )[0]
                for offset, r2sq in zip(angular_offsets_square, reduced_R2_square)
            ]
        )

    def PampelSuperLorentzian(
        self, T2: ndarray, offsets: float | ndarray, *args: Any, **kwargs: Any
    ) -> ndarray:
        """_summary_
        ...
        return sqrt(2 * pi) * array(
            [
                quad(
                    lambda cos_theta: self._Spherical(cos_theta, offset, r2sq, 985.96),
                    0,
                    1,
                )[0]
                for offset, r2sq in zip(angular_offsets_square, reduced_R2_square)
    ...
    def Cylindrical(
        ...
        if self.axonal_angle == 0:
            ...
            return sqrt(2 * pi * T2_totSquare) * exp(
                -0.5 * angular_offsets_square * T2_totSquare
            )

        sin_theta = -sin(deg2rad(self.axonal_angle))
        return sqrt(0.5 * pi) * array(
            [
                quad(
                    lambda cos_phi: self._Spherical(
                        sin_theta * cos_phi, offset, r2sq, 985.96
                    ),
                    -1,
                    1,
                )[0]
    ...
    def DispersedCylindrical(
        ...
        def ScaledBingham(theta: float, phi: float) -> float:
            ...
            for fiber in self.fibers:
                ...
                norm: float = (
                    f0 * 2 * pi * i0(0.5 * (k[0] - k[1])) * exp(0.5 * (k[0] + k[1]))
                ) ** 2
                values.append(norm * exp(dot(u, dot(mat, u))))
            return tuple(values)

        def Spherical(theta: float, phi: float, offsets: ndarray, T2: float) -> float:
            ...
            R2_neighboors = (
                31.4  # 10000000 would mean virtually no effect of T2_neighboors
            )
            R2_tmp = abs(3 * (-sin(theta) * cos(phi)) ** 2 - 1) / (2 * T2)
            T2_totSquare = 1 / (R2_neighboors * R2_neighboors + R2_tmp * R2_tmp)
            return sqrt(T2_totSquare) * exp(
                -0.5 * (2 * pi * offset * T2_totSquare) ** 2
            )

        norm = 1.0 / len(self.fibers)
        return (
            norm
            * sqrt(1.0 / (2 * pi) ** 3)
            * dblquad(
                lambda theta, phi: sin(theta)
                * Spherical(theta, phi, self.pulse.offset, T2)
                * sum(ScaledBingham(theta, phi)),
                0,
                pi,
                0,
                2 * pi,
    ...
    def _Spherical(
        u: float,
        angular_offset_square: float,
        reduced_R2_square: float,
        R2_neighbors_square: float,
    ) -> float:
        ...
        T2_total_square = 1 / (
            R2_neighbors_square + reduced_R2_square * (3 * u * u - 1) * (3 * u * u - 1)
        )
        return sqrt(T2_total_square) * exp(
            -0.5 * angular_offset_square * T2_total_square
        )
    ...
    def magnetization_recovery(self):
        if not hasattr(self, "_magnetization_recovery"):
            HomogenizeCol: ndarray = zeros(self.N_poolFree + 2 * (self.N_poolBound))
    ...
    def relaxation(self):
        if not hasattr(self, "_relaxation"):
            ...
            poolFree_block = diag(
                -(
                    1.0 / self.poolFree_T1
                    + sum(self.poolFreeBound_exchangeRate * self.poolBound_M0, axis=1)
                )
            )
            poolBound_block = diag(
                array(
                    [
                        [-(1.0 / T1 + exchange), 0]
                        for T1, exchange in zip(
                            self.poolBound_T1,
                            sum(
                                self.poolFreeBound_exchangeRate.T * self.poolFree_M0,
                                axis=1,
                            ),
                        )
                    ]
            ...
            for i in range(self.N_poolFree):
                for j in range(self.N_poolBound):
                    tmp[self.N_poolFree + 2 * j, i] = (
                        self.poolFreeBound_exchangeRate[i][j] * self.poolBound_M0[j]
                    )
                    tmp[i, self.N_poolFree + 2 * j] = (
                        self.poolFreeBound_exchangeRate[i][j] * self.poolFree_M0[i]
                    )

            # filling the diagonal relaxations: poolBound => dipolar compartment
            tmp[self.N_poolFree + 1 :: 2, self.N_poolFree + 1 :: 2] = diag(  # type: ignore
                -1.0 / self.poolBound_T1D
            )

            self._relaxation = tmp  # type: ignore
    ...
    def poolFree_Rrf(self):
        if not hasattr(self, "_poolFree_Rrf"):
            self._poolFree_Rrf = diag(
                [
                    *(
                        -0.5
                        * self.pulse.omegaRMS
                        * self.pulse.omegaRMS
                        * self.Lorentzian(self.poolFree_T2, self.pulse.offset)
                    ),
                    *zeros(2 * (self.N_poolBound)),
                ]
            )
    ...
    def poolFreeBound_exchangeRate(self, val: ScalarOrMatrix):
        check_value_is_valid(
            self, val, ScalarOrMatrix, [(lt, 0)], "poolFreeBound_exchangeRate"
        )
    ...
    def poolBound_lineshapeAsymmetry(self, val: ScalarOrVector):
        check_value_is_valid(
            self, val, ScalarOrVector, None, "poolBound_lineshapeAsymmetry"
        )
    ...
    def poolBound_omegaLocalField(self):
        if not hasattr(self, "_poolBound_omegaLocalField"):
            tmp = atleast_1d(1.0 / (_sqrt15 * self.poolBound_T2))
            tmp.setflags(write=False)
            check_value_is_valid(
                self, tmp, ScalarOrVector, [(lt, 0)], "poolBound_omegaLocalField"
            )
    ...
    def N_poolFree(self):
        if not hasattr(self, "_N_poolFree"):
            check_value_is_valid(
                self, len(self.poolFree_T2), int, [(lt, 1)], "N_poolFree"
            )
    ...
    def N_poolBound(self):
        if not hasattr(self, "_N_poolBound"):
            check_value_is_valid(
                self, len(self.poolBound_T2), int, [(lt, 1)], "N_poolBound"
            )
    ...
    def N_pools(self):
        if not hasattr(self, "_N_pools"):
            check_value_is_valid(
                self, self.N_poolFree + self.N_poolBound, int, [(lt, 1)], "N_pools"
            )
